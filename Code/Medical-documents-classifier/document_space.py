...
from pipeline_utils import DocLab
...
class DocumentSpace:
    ...
    def __init__(self, language_model=None, lang_mod_order=None, span=None):
        ...
        self.doc_lab = DocLab(self.span, vocab = lang_mod_order)

    def get_abs_vectors(self, papers, _from):
        ...
        :parameter from: either train or test, this is to diferentiate
        for later analisis
        """
        ...
        for paper in papers:
            ...
            self.doc_lab.add_doc(paper["id"], paper, _from)
            ...
            while (self.span and word_count < self.span) or (not self.span and word_count < len(words)):
                word = words[word_count]
                word_mtx.append(self.language_model[hash_table[word] if word in hash_table else 0]) ## append word vector, if the word isn't in the hash_table, append UNK vector
                word_count += 1
            try:
